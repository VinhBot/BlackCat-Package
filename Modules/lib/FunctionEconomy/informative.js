const { Currency: cs, Inventory: inv } = require("../Schema/OptionsSchema");
...
async function getInventory(settings) {
  ...
  let find = await inv.findOne({ guildID: settings.guild.id || null });
...
async function updateInventory( mongoURL, newData, settings, collection = "inventory-currencies") {
  ...
  if (settings.user)
    query = { userID: settings.user.id, guildID: settings.guild.id || null };
...
// ===================================================================
module.exports = { getUserItems, getShopItems, globalLeaderboard, leaderboard, balance, getInventory, makeInventory, updateInventory };