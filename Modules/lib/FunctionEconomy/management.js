const { Currency: cs } = require("../Schema/OptionsSchema");
...
async function saveUser(data, data2) {
  // điều này sẽ ngăn ngừa lỗi
  // ParallelSaveError: Không thể save() cùng một tài liệu nhiều lần song song.
...
async function transferMoney(settings) {
  ...
  const oldData = user1;
  let user2 = await cs.findOne({ userID: settings.user2.id, guildID: settings.guild.id || null });
...
module.exports = { removeMoney, addMoney, addMoneyToAllUsers, removeMoneyFromAllUsers, withdraw, deposite, gamble, setBankSpace, transferMoney, setDefaultBankAmount, setDefaultWalletAmount, setMaxBankAmount, findUser, makeUser, saveUser, amount };